...
export default function AdminProfile() {
  ...
  const btnClass =
    'px-5 py-2.5 bg-slate-950 text-white text-[14px] font-semibold rounded-xl hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2';
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col pt-6 px-4 sm:px-6 lg:px-10 pb-12">
        <div className="w-full max-w-lg mx-auto">
          {header}
          <div className="py-16 text-center text-slate-500 text-sm font-medium">로딩 중...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !profile) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col pt-6 px-4 sm:px-6 lg:px-10 pb-12">
        <div className="w-full max-w-lg mx-auto">
          {header}
          <div className="p-4 bg-red-50 text-red-700 rounded-2xl text-sm font-medium border border-red-100" role="alert">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pt-6 px-4 sm:px-6 lg:px-10 pb-12">
      <div className="w-full max-w-lg mx-auto">
        ...
        {mustChange && (
          <div
            className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-2xl text-sm"
            role="alert"
          >
            <p className="font-semibold">비밀번호 변경이 필요합니다</p>
            <p className="mt-1 text-amber-800">
              보안을 위해 새 비밀번호로 변경해 주세요. 변경 완료 전까지 다른 메뉴를 이용할 수 없습니다.
            </p>
          </div>
        )}
