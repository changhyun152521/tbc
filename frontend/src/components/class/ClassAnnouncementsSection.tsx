...
interface ClassAnnouncementsSectionProps {
  classId: string;
}

export default function ClassAnnouncementsSection({ classId }: ClassAnnouncementsSectionProps) {
  ...
  const fetchList = async () => {
    ...
    setError('');
    try {
      const res = await apiClient.get<{ success: boolean; data: AnnouncementItem[] }>(
        `/admin/classes/${classId}/announcements`
      );
      setList(res.data.success && Array.isArray(res.data.data) ? res.data.data : []);
    } catch {
      setError('공지 목록을 불러올 수 없습니다.');
      setList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await apiClient.put(`/admin/announcements/${editing._id}`, payload);
      } else {
        await apiClient.post(`/admin/classes/${classId}/announcements`, payload);
      }
      setFormOpen(false);
      await fetchList();
    } catch {
  ...
  return (
    <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="px-4 py-2 sm:px-5 lg:px-8 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-slate-600 text-sm sm:text-base font-bold uppercase tracking-wide">공지사항</h2>
        <button
          type="button"
          onClick={openCreate}
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-slate-400 text-sm">등록된 공지가 없습니다.</div>
      ) : (
              ...
              {list.map((row) => (
                <tr key={row._id} className="hover:bg-slate-50 text-slate-700">
                  ...
                  <td className="py-3 px-4 text-center whitespace-nowrap space-x-3">
                    <button type="button" onClick={() => openEdit(row)} className="text-slate-600 hover:text-slate-950">
                      수정
                    </button>
                    <button type="button" onClick={() => handleDelete(row._id)} className="text-slate-400 hover:text-red-600">
                      삭제
                    </button>
                  </td>
                </tr>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50" onClick={() => setFormOpen(false)}>
          <form
            className="bg-white border border-slate-200 rounded-2xl shadow-lg p-6 max-w-md w-full"
            onClick={(e) => e.stopPropagation()}
            onSubmit={handleSubmit}
          >
            <h3 className="text-lg font-bold text-slate-950 mb-4">{editing ? '공지 수정' : '공지 등록'}</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input type="date" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">종료일</label>
                <input type="date" value={endsAt} onChange={(e) => setEndsAt(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg" required />
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setFormOpen(false)} className="flex-1 py-2 border border-slate-200 rounded-lg text-slate-700">
                취소
              </button>
              <button type="submit" disabled={saving} className="flex-1 py-2 bg-slate-950 text-white rounded-lg disabled:opacity-50">
                {saving ? '저장 중...' : '저장'}
              </button>
            </div>
          </form>
