...
| **DB** | MongoDB (로컬: `mongodb://localhost:27017/tbc-class`, Atlas: `.env`의 `MONGODB_URI`) |

### 1.4 실행 방법

- **백엔드**: `cd backend && npm run dev` → `http://localhost:3001`
  - MongoDB Atlas 사용 시: `backend/.env`에 `MONGODB_URI` 설정 (`.env.example` 참고)
...
- **serverConfig.port**: `PORT` 환경변수 또는 `3001`
- **dbConfig.uri**: `MONGODB_URI` 환경변수 또는 `mongodb://localhost:27017/tbc-class`
- **jwtConfig**: `JWT_SECRET`, `JWT_EXPIRES_IN` (기본 24h)

MongoDB Atlas 사용 시 `backend/.env` 파일 생성 후 아래 설정:

```
MONGODB_URI=mongodb+srv://<user>:<password>@<cluster>.mongodb.net/tbc-class
```

`.env.example`을 복사하여 `.env`로 저장 후 값을 채워 넣으면 됩니다.
